...
const getContext = async (userId) => {
    ...
    const [revResult, lowStock, overdueDebts, health, credit] = await Promise.all([
        ...
        Inventory_1.default.find({ userId: uid, $expr: { $lte: ['$quantity', '$lowStockThreshold'] } }).select('name quantity').limit(5).lean(),
        Debt_1.default.find({ userId: uid, status: { $in: ['pending', 'partial'] }, dueDate: { $lt: new Date() } }).select('customerName balance').limit(5).lean(),
        HealthScore_1.default.findOne({ userId: uid }).sort({ calculatedAt: -1 }).lean(),
        CreditScore_1.default.findOne({ userId: uid }).sort({ calculatedAt: -1 }).lean(),
    ]);
    const monthlyRev = await Transaction_1.default.aggregate([
...
const generateInsight = (message, ctx) => {
    const msg = message.toLowerCase();
    if (msg.includes('sale') || msg.includes('revenue') || msg.includes('income') || msg.includes('money')) {
        return ctx.todayRevenue > 0
            ? `Today you've made ₦${ctx.todayRevenue.toLocaleString()} from ${ctx.todaySales} sale(s). This month your total revenue is ₦${ctx.monthlyRevenue.toLocaleString()}. ${ctx.monthlyRevenue > 50000
    ...
    if (msg.includes('stock') || msg.includes('inventory') || msg.includes('product')) {
        if (ctx.lowStock.length > 0) {
            const items = ctx.lowStock.map((i) => i.name).join(', ');
    ...
    if (msg.includes('debt') || msg.includes('owe') || msg.includes('credit') || msg.includes('loan')) {
        ...
        return `Your debt book is clean — no overdue records. Your credit score is ${ctx.creditScore}/850. ${ctx.creditScore >= 670
    ...
    if (msg.includes('score') || msg.includes('health') || msg.includes('business')) {
        return `Your Business Health Score is ${ctx.healthScore}/100 and Credit Score is ${ctx.creditScore}/850. ${ctx.healthScore >= 75
            ? 'Strong health score! Share your Business Passport with lenders and partners to prove your credibility.'
            : 'To improve your health score: record daily transactions, manage inventory, and settle debts on time.'} Monthly revenue this period: ₦${ctx.monthlyRevenue.toLocaleString()}.`;
    }
    if (msg.includes('tip') || msg.includes('advice') || msg.includes('improve') || msg.includes('grow') || msg.includes('increase')) {
        const tips = [];
        if (ctx.lowStock.length > 0)
            tips.push(`Restock ${ctx.lowStock[0].name} to avoid lost sales`);
        if (ctx.overdueDebts.length > 0)
            tips.push(`Collect overdue payment from ${ctx.overdueDebts[0].customerName}`);
        if (ctx.healthScore < 75)
            tips.push('Record at least 1 transaction daily to boost your health score');
        if (ctx.creditScore < 670)
            tips.push('Settle debts on time to build your credit score above 670');
        if (tips.length === 0)
            tips.push('Keep up the great work — share your Business Passport with potential partners');
    ...
    // Default
    return `Hello! I'm your StreetOS advisor. Today's revenue: ₦${ctx.todayRevenue.toLocaleString()} | Health: ${ctx.healthScore}/100 | Credit: ${ctx.creditScore}/850. ${ctx.lowStock.length > 0 ? `⚠️ ${ctx.lowStock.length} item(s) low on stock. ` : ''}${ctx.overdueDebts.length > 0 ? `⚠️ ${ctx.overdueDebts.length} overdue debt(s). ` : ''}Ask me about your sales, inventory, debts, scores, or tips to grow your business.`;
};
const chat = async (req, res) => {
    ...
    (0, response_1.sendSuccess)(res, { response }, 'AI response');
};
exports.chat = chat;
const getDailyBriefingHandler = async (req, res) => {
    ...
    const parts = [
        ...
        ctx.lowStock.length > 0 ? `⚠️ Low stock: ${ctx.lowStock.map((i) => i.name).join(', ')}.` : `📦 Inventory levels look good.`,
    ...
    (0, response_1.sendSuccess)(res, { briefing: parts.join(' ') }, 'Daily briefing');
};
